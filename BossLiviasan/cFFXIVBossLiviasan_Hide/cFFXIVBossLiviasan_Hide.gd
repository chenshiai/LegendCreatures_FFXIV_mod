...
{c_skill}[裂流]{/c}：{TDeath}对仇恨目标发射激流，造成{c_mgi}[{1}]{/c}的魔法伤害，并附加[水耐性下降·大]，连续使用两次
{c_skill}[怒潮咆哮]{/c}：对全屏的敌人造成{c_mgi}[{2}]{/c}的魔法伤害
{c_skill}[离岸洋流]{/c}：对场地一边进行喷水，对命中的敌人造成{c_mgi}[{3}]{/c}的魔法伤害，并附加[水耐性下降·大]
{c_skill}[大漩涡]{/c}：随机出现在场地一边，进行半场俯冲对命中的敌人造成{c_mgi}[{4}]{/c}的魔法伤害，并附加[水耐性下降·大]{/c}"""
