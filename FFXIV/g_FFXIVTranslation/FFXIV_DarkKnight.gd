...
[噬魂斩]：{TPassive}第三次普通攻击造成{c_phy}[110%]{/c}的伤害，并恢复自身{c_tre}5%{/c}的HP
[血溅]：冷却10s，对目标造成{c_phy}[290%]{/c}的物理伤害"""
const SKILL_EN = """[Grit]：When battle start. Magic defense increased{c_base}(10%){/c}. Reduce damage taken{c_base}(20%){/c}
[Souleater]：When 3rd normal attack. Damage increased to {c_phy}[110%]{/c} and recover HP{c_tre}(5%){/c}
...
[ソウルイーター]：第三次通常攻撃で{c_phy}[110%]{/c}のダメージを与え、自身の{c_tre}5%{/c}のHPを回復する
...
const SKILL_2_CN = "[掠影示现]：{TPassive}令英雄的掠影变为实体与自身并肩作战，与本体同步攻击，造成的伤害提高[30%]，自身最大生命值提高"
